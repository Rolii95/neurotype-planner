#!/usr/bin/env node
/* CommonJS wrapper for verify_metrics_db to run in projects with "type": "module" */
...
async function existsInInformationSchema(kind, name) {
  ...
  try {
    const { data, error } = await supabase.from(schemaTable).select('table_name').eq('table_name', name).limit(1);
    if (error) {
